...
std::string createModtStr(Server *srv, const int &fd, std::string &filename)
{
    ...
    replyMsg.append(numericReply(srv, fd, "372", RPL_MOTD(std::string("Hi My name is Arafa, Welcome"))));
	try {
		while (std::getline(infile, line))
            replyMsg.append(numericReply(srv, fd, "372", RPL_MOTD(line)));
	} catch (std::ios_base::failure &e) {}
    replyMsg.append(numericReply(srv, fd, "372", RPL_MOTD(std::string("Hola Tats here"))));
